...
def run_multiple_catalogs(file_list, labels, args):
    """
    Run anomaly detection on multiple catalogs and plot comparison.
    """
    all_results = []
    ...
    for file, label in zip(file_list, labels):
        ...
        times, lat, lon, dep, mag = parse_catalog(file, mc=args.mc)
        ...
        all_results.append((t_cent, score, label))

    # ---- Plot all together ----
    fig, ax = plt.subplots(figsize=(10*0.7,5*0.7))

    for spine in ax.spines.values():
        spine.set_visible(True)
        spine.set_linewidth(1.0)

    for t_cent, score, label in all_results:
        ax.plot(t_cent, score, linewidth=1.2, label=label)
    ...
    fig.subplots_adjust(left=0.12, right=0.98, bottom=0.15, top=0.95)
    fig.savefig(f"{args.out_prefix}_compare.png", dpi=500)
    fig.savefig(f"{args.out_prefix}_compare.svg", bbox_inches="tight")
    fig.savefig(f"{args.out_prefix}_compare.eps", format="eps", bbox_inches="tight")
    plt.show()
...
def main():
    ...
    # -----------------------------
    # MULTI-CATALOG MODE (automatic)
    # -----------------------------

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

    catalogs = [
        os.path.join(BASE_DIR, "..", "catalogs", "catalog.txt"),
        os.path.join(BASE_DIR, "..", "catalogs", "catalog_rate_bump2.txt"),
        os.path.join(BASE_DIR, "..", "catalogs", "catalog_rate_bump3.txt"),
        os.path.join(BASE_DIR, "..", "catalogs", "catalog_rate_bump4.txt"),
        os.path.join(BASE_DIR, "..", "catalogs", "catalog_rate_bump5.txt"),
        os.path.join(BASE_DIR, "..", "catalogs", "catalog_rate_bump6.txt"),
    ]

    labels = ["Original", "Sim1", "Sim2", "Sim3", "Sim4", "Sim5"]

    run_multiple_catalogs(catalogs, labels, args)
    t_days = times_to_days(times)
